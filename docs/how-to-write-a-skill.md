...
| File | Role |
|------|------|
| `SKILL.md` | Human- and runtime-readable contract. Read before execution. |
...
Operators and runtimes read `SKILL.md` before execution. Keep it accurate because it is the source of truth for what a skill does.
...
Add an entry to `SKILL_REGISTRY` in `src/agentic_os/control_plane/registry.py`:

```python
from agentic_os.control_plane.registry import SkillEntry
...
The policy engine in `src/agentic_os/control_plane/policy.py` governs what skills are allowed to do. You cannot override it from inside a skill.
...
Skills that return paths outside `vault/` or `data/` are blocked by the runner before any write occurs. Keep all output paths inside the permitted prefixes.
...
**`src/agentic_os/control_plane/registry.py` entry**
